                ...
                using (var conn = new SqlConnection(connectionString))
                {
                    ...
                    if (SelectedTicket.Id == 0) // Новый билет
                    {
                        var cmd = new SqlCommand(
                            @"INSERT INTO Tickets ([From], [To], Date, Time, Price, Number, Description, Type, BoardingPoints, DropOffPoints)
                            VALUES (@From, @To, @Date, @Time, @Price, @Number, @Description, @Type, @BoardingPoints, @DropOffPoints);
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        tickets.Add(new Ticket
                        {
                            ...
                            Type = displayType,
                            BoardingPoints = reader.GetString(9),  // Исправлено: индекс 9 для BoardingPoints
                            DropOffPoints = reader.GetString(10)   // Исправлено: индекс 10 для DropOffPoints
                        });
                    }
                }
            }
