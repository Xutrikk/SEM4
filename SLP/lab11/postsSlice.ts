...
export const createPostThunk = createAsyncThunk('posts/createPost', async (post: NewPost) => {
...
export const updatePostThunk = createAsyncThunk('posts/updatePost', async (post: Post) => {
  ...
  try {
    if (post.id < 0) {
      ...
      const response = await axios.put('https://jsonplaceholder.typicode.com/posts/101', postData);
      console.log('updatePostThunk: API response:', response.data);
      const result = { id: post.id, title: post.title, body: post.body, userId: post.userId };
...
export const deletePostThunk = createAsyncThunk('posts/deletePost', async (id: number) => {
  try {
    if (id < 0) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updatePostThunk.fulfilled, (state, action) => {
        console.log('updatePostThunk.fulfilled: Updating state with:', action.payload);
        const index = state.posts.findIndex((post) => post.id === action.payload.id);
        console.log('updatePostThunk.fulfilled: Found index:', index);
        if (index !== -1) {
          state.posts[index] = action.payload;
          console.log('updatePostThunk.fulfilled: Updated post:', state.posts[index]);
          console.log('updatePostThunk.fulfilled: Current posts state:', state.posts);
        } else {
